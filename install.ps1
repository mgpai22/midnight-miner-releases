#!/usr/bin/env pwsh
# install.ps1 — download & install the right midnight-miner binary for this machine.
# Usage: .\install.ps1 -Url <SERVER_URL>
#   env: $env:BIN_DIR (default: ~/.local/bin on Linux/macOS, ~\AppData\Local\Programs on Windows)
#        $env:NAME (installed executable name, default: midnight-miner)
#
...
#   midnight-miner-rs-windows-x64.zip
#   midnight-miner-rs-windows-arm64.zip
...
if ($Help) {
    ...
    Write-Host @"
Usage: .\install.ps1 -Url <SERVER_URL> [-Local]
...
if (-not $Url) {
    Write-Error-Exit "-Url is required (e.g. -Url https://example.com/releases/bins)"
}
...
$file = "midnight-miner-rs-$target.$extension"
$downloadUrl = "$($Url.TrimEnd('/'))/$file"
...
$tmpDir = Join-Path $env:TEMP "midnight-miner-install-$(Get-Random)"
...
try {
    ...
    $binSrc = Get-ChildItem -Path $tmpDir -Recurse -File -Depth 3 |
        Where-Object { $_.Name -like "midnight-miner*" -and ($_.Extension -eq ".exe" -or $_.Extension -eq "" -or $_.UnixFileMode) } |
